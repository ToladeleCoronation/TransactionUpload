...
public class Utilities {
    ...
    @Value("${app.lientTransfer}")
    private String lienTransfer;

    @Value("${app.lienRemove}")
    private String lienRemove;
    ...
    public ResponseEntity<ADLoginResponse> adLogin(ADLogin login) {
        login.setAppid(adAppId);
        String password = adAppId + login.getUsername() + login.getPasswd() + adSecret;
        String hashedPassword = sha512Encrypt(password);
        login.setHash(hashedPassword);
        javax.net.ssl.HttpsURLConnection.setDefaultHostnameVerifier(
                new javax.net.ssl.HostnameVerifier() {

                    public boolean verify(String hostname,
                                          javax.net.ssl.SSLSession sslSession) {
                        return hostname.equals(adDomain);
                    }
                });
        return makePostCall(adUrl, login, ADLoginResponse.class);
    }
    ...
    public ResponseEntity<TransferResponses> postTransfer(TransferRequestt request) {
        return makePostCall(transferUrl, request, TransferResponses.class);
    }

    public ResponseEntity<LienResponse> postLien(LienRequest request) {
        return makePostCall(lienTransfer, request, LienResponse.class);
    }
    public ResponseEntity<LienResponse> removeLien(LienRequest request) {
        return makePostCall(lienRemove, request, LienResponse.class);
    }
